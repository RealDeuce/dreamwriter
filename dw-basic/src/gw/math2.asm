...
;THAN 1000000.
	MOV	word [DOL_DBUFF+4],22000
	MOV	word [DOL_DBUFF+6],112164
	MOV	BX,DOL_DBUFF+6
...
FI30:	CALL	DOL_MUL10 ;MULTIPLY $FAC BY 10
	POP	DX ;RECALL DIGIT
	PUSH	word [DOL_FACLO]
	PUSH	word [DOL_FAC-1] ;FAC PUSHED ON STACK
...
DOL_FINEX: LAHF ;SAVE STATUS
	CMP	byte [DOL_VALTP],10 ;SEE IF ALREADY D.P.
	JNZ	EXA
...
FX30:	PUSH	AX ;SAVE NEW DIGIT
	...
	SUB	DL,60 ;SUBTRACT OUT ASCII BIAS
...
DOL_FIND: ;FORCE INPUT TO DOUBLE PRECISION
	OR	AL,1 ;TO SIGNAL DOUBLE PRECISION
...
DOL_CSD: ;CONVERT DOUBLE TO SINGLE PRECISION
	MOV	AL,4 ;SINGLE PREC DESIGNATION
	MOV	byte [DOL_VALTP],AL
	MOV	BL,byte [DOL_FAC-1] ;FETCH HIGH MANTISSA BITS
	MOV	byte [DOL_FAC+1],BL ;MOVE SIGN TO $FAC+1
	MOV	DX,word [DOL_FAC-3] ;FETCH REST OF MANTISSA
	MOV	AH,byte [DOL_FAC-4] ;FETCH OVERFLOW BITS
	OR	AH,100 ;WANT ROUND-UP IF HIGH BIT SET
	OR	BL,200 ;PUT IN UNDERSTOOD 1
...
VALINT:
DOL_VALNT: MOV	byte [DOL_VALTP],2
	RET
...
DOL_FLT:	XOR	BX,BX ;CLEAR HIGH MANTISSA BYTE (BL)
	...
	MOV	SI,DOL_FAC+1 ;FETCH $FAC ADDRESS TO (SI)
	MOV	byte [SI-1],220 ;SET EXPONENT TO 16
	MOV	byte [SI+0],0 ;SET SIGN POSITIVE
	OR	DX,DX ;SETS SF=1 IF NEGATIVE NO.
	JNS	FLT10 ;IF POSITIVE PROCEED
	NEG	DX ;NEED POSTIVE MAGNITUDE
	MOV	byte [SI+0],200 ;SET SIGN TO NEGATIVE
FLT10:	MOV	BL,DH ;WILL MOVE (DX) TO (BLDH)
	MOV	DH,DL ;
	MOV	DL,BH ;SET (DL)=0
	MOV	byte [DOL_VALTP],4 ;SET TYPE TO S.P.
...
FMD10:	RET
FMD20:
	MOV	BX,word [DOL_ARG-1] ;FETCH SIGN AND EXP. TO BX
	CALL	DOL_AEXPS ;ADD THE EXPONENTS
	PUSH	word [DOL_FAC] ;EXPONENT,SIGN
	MOV	word [DOL_ARG-1],BX ;REPLACE UNPACKED MANTISSA
...
M1:	MOV	AX,word [BX+SI+0] ;FETCH MULTIPLIER V(J)
	OR	AX,AX ;SEE IF ZERO
	JZ	M4D ;IF ZERO W(J)=0
	MOV	DI,0 ;I
	MOV	CX,DI ;K
M4:	MOV	AX,word [BX+SI+0] ;FETCH MULTIPLIER V(J)
	MUL	word [BP+DI+0] ;FORM PRODUCT V(J)*U(J) IN (DXAX)
	PUSH	BX ;SAVE PTR. TO MULTIPLIER BASE
	MOV	BX,SI ;
	ADD	BX,DI ;I+J
	ADD	BX,DOL_DFACL-10 ;W(I+J) ADDRESS IN BX
	ADD	AX,word [BX+0] ;(DXAX)=U(I)*V(J)+W(I+J)
	JNB	M4A
	INC	DX
M4A:	ADD	AX,CX ;T=U(I)*V(J)+W(I+J)+K
	JNB	M4B
	INC	DX
M4B:	MOV	word [BX+0],AX ;W(I+J)= T MOD 2^16
...
M4D:	PUSH	BX ;SAVE PTR TO MULTIPLIER BASE
	MOV	BX,DOL_DFACL
	MOV	word [BX+SI+0],AX ;W(J)=K OR 0 (0 IF V(J) WERE 0)
	POP	BX ;RECALL PTR TO MULTIPLIER BASE
...
M5: ;MULTIPLICATION COMPLETE AND IN FAC
	MOV	SI,DOL_DFACL-2 ;WILL NOW SET ST
...
M5AA:	LODSB ;FETCH NEXT BYTE
	...
	OR	byte [DOL_DFACL-1],40 ;"OR" IN ST BIT
M5AB:
	MOV	AL,byte [DOL_FAC-1] ;SEE IF WE NEED TO INC EXPONENT
	OR	AL,AL
	POP	word [DOL_FAC] ;RESTORE EXPONENT,SIGN
	JS	M6
	MOV	BX,DOL_DFACL-1 ;MUST SHIFT 1 BIT LEFT
	MOV	CX,4
M5A:	RCL	word [BX+0],1
	INC	BX
	INC	BX
...
;MULTIPLY.
;***************************************************************
	MOV	CX,word [DOL_FAC-1] ;(CH)=($FAC):(CL)=($FAC-1)
	XOR	CH,CH ;(CX) CONTAINS HIGH MANTISSA BITS
	MOV	AX,word [DOL_FAC-3] ;(AX) CONTAINS LOW MANTISSA BITS OF FAC
...
DOL_FOTAN:
	...
	MOV	byte [BX+0],"." ;MOVE IN DECIMAL POINT
FTN10:	INC	BX ;POINT TO NEXT OUTPUT POSITION
	MOV	byte [BX+0],"0" ;PUT IN LEADING ZERO
...
FCV10:	CALL	DOL_FOTED ;SEE IF DECIMAL POINT NEEDED
	...
	MOV	AL,"0"-1 ;WILL FORM DIGIT IN (AL)
	PUSH	AX
...
;AND ADVANCE SI TO NEXT POWER 10
	...
	MOV	byte [BX+0],AL ;PUT IN ASCII DIGIT
	INC	BX ;POINT TO NEXT BUFFER POSITION
...
FCV60:	CALL	DOL_FOTED ;SEE IF NEED A DECIMAL POINT OR COMMA
	...
	MOV	AL,"0"-1 ;WILL BUILD DIGIT IN (AL)
	PUSH	AX ;SAVE DIGIT
...
;POWER TEN TABLE IN CODE SEGMENT
	ADD	DX,word [CS:BP+0] ;ADD WORD PORTION
	ADC	BL,byte [CS:BP+2]
;SINCE WE SUBTRACTED ONE TOO MANY
	...
	MOV	byte [BX+0],AL ;SEND OUT DIGIT
	...
	MOV	AH,4 ;CONVERT ONLY 4 DIGITS
	JMP	DOL_FCI4
;INTEGER ARITHMETIC
...
FNV20:	MOV	BX,DOL_DP06+4 ;LOWER BOUND
...
;
;****************************************************************
S:	MOV	BX,DOL_FBUFF+1 ;FETCH BUFFER POINTER
	MOV	byte [BX+0]," " ;MOVE IN SPACE FOR POSSIBLE SIGN
	PUSH	BX ;SAVE BUFFER POINTER
	CALL	DOL_SIGNS ;DETERMINE SIGN OF NUMBER
	POP	BX ;RECALL BUFFER POINTER
	PUSHF ;SAVE FLAGS FOR LATER
	JNS	FO20 ;JUMP IF POSITIVE
	MOV	byte [BX+0],"-" ;PUT IN MINUS SGN
	PUSH	BX ;SAVE TEXT POINTER
	CALL	DOL_VNEG ;NEGATE NO. SO WE WORK ONLY WITH
	POP	BX ;RECALL TEXT POINTER
	OR	AL,1 ;POS. NOS. AND SET ZF=0
FO20:	INC	BX ;POINT TO NEXT BUFFER POSITION
	MOV	byte [BX+0],"0" ;PUT IN ZERO IN CASE NO IS ZERO
	POPF ;RECALL FLAGS
	RET
FOUT:
DOL_FOUT: ;FREE-FORMAT ENTRY POINT
	...
	MOV	byte [BX+0],0 ;INDICATE END OF NUMBER
	MOV	BX,DOL_FBUFF+1 ;POINT (BX) TO START POSITION
	RET
DOL_FOUT2: CALL	DOL_GETYP ;GET TYPE NO.
	JNS	FO50 ;GO FORMAT SINGLE OR DOUBLE PREC.
	MOV	CX,7*400 ;default 7 digits prior to dp.
...
DOL_DINT: ;DOUBLE PRECISION INT FUNCTION
	...
	SUB	CL,270 ;Is there a fractional part?
...
DINTNZ: MOV	BX,DOL_FAC-2
	MOV	AL,byte [BX+1] ;FETCH SIGN BYTE
	MOV	byte [BX+3],AL ;AND PUT IN $FAC+1  FOR $NORMD
	OR	AL,AL ;SEE IF NEGATIVE
	PUSHF
	OR	AL,200 ;RESTORE HIDDEN 1
	MOV	byte [BX+1],AL ;AND REPLACE
	MOV	byte [BX+2],270 ;SET EXPONENT FOR POST SHIFT VALUE
	POPF ;RECALL SF
	PUSHF
...
DNT15:	MOV	byte [DOL_DFACL-1],0 ;ZERO THE OVERFLOW BYTE
...
DINA10: SUB	word [BX+0],1 ;NEED CF SO CAN'T USE DEC
	JNB	DINA20
	INC	BX
...
DOL_INT: ;SINGLE PRECISION INT FUNCTION
	...
	SUB	CL,230 ;CALCULATE SHIFT COUNT
...
QINTX: ; $QINTX'S ENTRY POINT
	...
	MOV	BX,word [DOL_FAC-1] ;FETCH EXP,SIGN,HIGH MANTISSA BITS
...
;this is a QINT call.
	...
	PUSHF ;SAVE
	MOV	byte [DOL_FAC+1],BL ;SAVE SIGN FOR NORMS
	MOV	byte [DOL_FAC],230 ;SET EXP FOR POST SHIFT
	OR	BL,200 ;RESTORE HIDDEN 1
	POPF
	PUSHF ;SAVE SIGN
	JNS	INT10 ;
	SUB	DX,1 ;CAN'T DO A 'DEC' BECAUSE NEED CF
	SBB	BL,0 ;DO APPROPRIATE SUBTRACT TO HIGH BYTE
...
NGBLDX: JNS	INT30 ;IF NOT PROCEED AS NORMAL
	...
	ADC	BL,0 ;2's COMPLEMENT NOW FORMED
INT30:	RET
QINT:
...
LOG10:
	...
	SUB	AL,200 ;TAKE OUT BIAS
	CBW ;CONVERT BYTE TO WORD
	PUSH	AX ;AND SAVE
	MOV	byte [DOL_FAC],200 ;ZERO THE EXPONENT
...
;
;*******************************************************************
DOL_NORMD: MOV	DL,71 ;MAX BITS TO SHIFT LEFT
	MOV	BX,DOL_DFACL-1
NORD5:	MOV	DI,DOL_FAC-1
	MOV	SI,DOL_FAC
	JMP	NORD30
NORD10:
	MOV	CX,4
	CLC ;CF=0
NORD20: RCL	word [BX+0],1
	INC	BX
	INC	BX ;POINT TO NEXT WORD
	LOOP	NORD20
	MOV	BX,DOL_DFACL-1 ;POINT BACK TO END OF NUMBER
NORD25:
	DEC	byte [SI+0] ;DECREMENT EXPONENT
	JZ	NORD40 ;DO CLEAN-UP IF UNDERFLOW
	DEC	DL ;SEE IF MAX BITS SHIFTED
	JZ	NORD40 ;IF SO TERMINATE SHIFTS
NORD30: TEST	byte [DI+0],377 ;SF=1 IF NOW NORMALIZED
	JS	NORD40 ;NORMALIZED
	JNZ	NORD10 ;MUST SHIFT BIT AT A TIME
;***************************************************************
;CAN DO AT 1 BYTE MOVE LEFT
;***************************************************************
	SUB	byte [SI+0],10 ;SUBTRACT 8
	JBE	NORD40 ;UNDERFLOW
	SUB	DL,10 ;SEE IF MAX BITS SHIFTED
...
;REPEAT CX TIMES (THE MOVB)
 REP	MOVSB ;MOVE
	MOV	byte [DOL_DFACL-1],0 ;ZERO OVERFLOW
...
NOR10:	OR	BL,BL ;SEE IF SIGN BIT SET
	...
	SUB	BH,10 ;CAN WE SUBTRACT 8 W/O UNDERFLOW?
	JBE	NOR17
	MOV	BL,DH
	MOV	DH,DL
	MOV	DL,AH
...
;*******************************************************************
	...
	MOV	byte [BX+0],"%" ;PUT IN OVERFLOW INDICATOR
	RET ;DONE
PUF30:
;********************************************************************
...
;********************************************************************
	...
	MOV	CH,20 ;ASSUME DOUBLE PREC. (16 PRINTABLE POS)
	JNB	PUF40
	MOV	CH,7 ;7 print positions if single precision
...
;TO ACCOMODATE THIS NUMBER.
	SUB	AL,byte [DOL_FMTCX+1] ;SUBTRACT DIGITS TO LEFT
...
;********************************************************************
	MOV	AL,byte [DOL_FMTCX+1] ;DESIRED LEADING DIGITS
	CALL	DOL_FOTZ ;INSERT (AL) LEADING ZEROS
	MOV	byte [BX+0],"." ;PUT IN DECIMAL POINT
...
;*********************************************************************
	MOV	AL,byte [DOL_FMTCX+1] ;FETCH DESIRED DIGITS
...
DPCOM: ;ROUTINE TO SET UP DECIMAL POINT AND COMMA COUNTS
	...
	INC	AL ;FOR SIGN
	MOV	CH,AL
DPC10:	SUB	AL,3 ;SUBTRACT OUT AL MULTIPLES OF 3
	JG	DPC10
	ADD	AL,3
	MOV	CL,AL ;COMMA COUNT NOW SET
	MOV	AL,byte [DOL_FMTAX] ;FETCH FORMAT FLAGS
	AND	AL,100 ;SEE IF COMMAS DESIRED
	JNZ	DPC20
...
DOL_PUFOT:
	...
	TEST	AH,100 ;ZF=0 IF COMMAS DESIRED
	MOV	AH,3 ;IF COMMAS DESIRED
...
;OF DECIMAL POINT
	...
	MOV	BX,DOL_FBUFF+1 ;FETCH BUFFER POINTER
	MOV	byte [BX+0]," " ;MOVE IN SPACE FOR POSSIBLE SIGN
	TEST	AH,10 ;SEE IF "+" DESIRED
	JZ	PFO10 ;JUMP IF NOT
	MOV	byte [BX+0],"+" ;MOVE IN PLUS
...
;IN (CL)
	MOV	CH,6 ;AT MOST 6 PLACES PRIOR TO DECIMAL PT
...
PI10:	MOV	AL,DH ;FETCH PLACES TO LEFT OF D.P.
	SUB	AL,5 ;LET'S SEE IF LEADING ZEROS NEEDED
;*******************************************************************
...
;OF DECIMAL POINT.
	...
	MOV	byte [BX+0],0 ;RE-ESTABLISH END-OF-PRINT
...
DOL_RND:	CALL	DOL_SIGNS ;FIND WHICH MODE REQUESTED
	...
	MOV	AL,byte [DOL_FAC-1]
	MOV	byte [DOL_RNDX+2],AL ;SO SEQUENCE STARTS AGAIN
RND10:	MOV	AX,word [DOL_RNDX] ;FETCH LOW 16 BITS OF RNDX
	...
	MOV	AL,byte [DOL_RNDA+2] ;FETCH UPPER 8 BITS OF A
	XOR	AH,AH ;CLEAR UPPER AX
	MUL	word [DOL_RNDX] ;MULTIPLY BY LOW RNDX
	ADD	CL,AL ;ADD TO UPPER 8 BITS
	XOR	AH,AH ;CLEAR UPPER AX
	MOV	AL,byte [DOL_RNDX+2] ;FETCH HIGH 8 BITS OF RNDX
	...
	MOV	BL,byte [DOL_RNDC+2] ;FETCH HIGH OF C
	ADC	BL,CL ;ADD WITH CARRY HIGH RND(N)*A
	MOV	byte [DOL_FAC+1],AH ;SIGN IS POSITIVE
	MOV	AL,200 ;SO NORM WILL PRODUCE NO.
;LESS THAN 1
	MOV	byte [DOL_FAC],AL
	MOV	word [DOL_RNDX],DX ;SAVE NEW RND(N+1)
	MOV	byte [DOL_RNDX+2],BL
	MOV	AL,4 ;MUST SET VALTP TO SINGLE PREC.
	MOV	byte [DOL_VALTP],AL
	JMP	DOL_NORMS
NEWSD: ;DETERMINE NEW RANDOM NO. SEED
	MOV	BX,DOL_FBUFF ;WILL SUM THE FIRST 32 WDS OF
	MOV	CX,40 ;RAM
NEW10:	ADD	AX,word [BX+0]
	INC	BX
	INC	BX
	LOOP	NEW10
	AND	AL,376 ;BE SURE BIT 0=0 SO AS TO NOT
;MATCH RND0
...
OLDRN: ;RETURN THE PREVIOUS RANDOM NO.
	...
	MOV	BL,byte [DOL_RNDX+2] ;FETCH HIGH 8 BITS
	XOR	AX,AX ;CLEAR OVERFLOW BYTE (AH)
	MOV	AL,200 ;EXPONENT OF 0
	MOV	byte [DOL_FAC],AL
	MOV	byte [DOL_FAC+1],AH ;POSITIVE SIGN
	JMP	DOL_NORMS ;GO NORMALIZE
...
DOL_ROUND: ;DOUBLE PRECISION ROUND AND PACK
	MOV	BX,DOL_DFACL-1
	ADD	word [BX+0],200 ;ADD TO HIGH BIT OV OVERFLOW BYTE
	MOV	CX,3 ;3 MORE BYTES TO LOOK AT POTENTIALLY
	JNB	RDD20 ;IF CF=0 WE ARE DONE
RDD10:	INC	BX
	INC	BX
	INC	word [BX+0] ;IF THIS GETS ZF=1 THEN CARRY
	JNZ	RDD20 ;FINISHED WHEN ZF=0
	LOOP	RDD10
	INC	byte [DOL_FAC] ;MUST INCREMENT EXPONENT
	RCR	word [BX+0],1 ;SET HIGH BYTE TO 200
RDD20:
	JZ	RDD30 ;OVERFLOW HOOK
	TEST	byte [DOL_DFACL-1],377 ;SEE IF OVERFLOW BYTE ZERO
	JNZ	DOL_ROUNX
	AND	byte [DOL_DFACL],376 ;MAKE ANSWER EVEN
DOL_ROUNX:
	AND	byte [DOL_FAC-1],177 ;CLEAR SIGN BIT
	MOV	AL,byte [DOL_FAC+1] ;FETCH SIGN BYTE
	AND	AL,200 ;CLEAR ALL BUT SIGN
	OR	byte [DOL_FAC-1],AL ;AND SET SIGN APPROPRIATELY
	RET
RDD30:	JMP	DOL_OVFLS
...
;
;**************************************************************
DOL_ROUNS: AND	AH,340 ;CLEAR SUPERFLUOUS BITS
DOL_ROUNM: ADD	AH,200 ;ADD TO MOST SIG. BIT OF AH
...
;*****************************************************************
	...
	CMP	byte [DOL_FAC],270 ;IF EQUAL OR SF=0 THEN INTEGER ALREADY
	JNS	SI20 ;DON'T ADD .5
	JMP	SI32 ;PROCESS FAC NORMALLY
SI31:	CMP	byte [DOL_FAC],230 ;IF SF=0 THEN INTEGER ALREADY
	JNS	SI20
...
SI30:	PUSH	CX ;SAVE CX FOR LATER
	...
	MOV	BX,DOL_Z1-1 ;ADDRESS OF Z1-1
